...
function Install()
{
    ...
    then
	apt update && apt install gns3-gui tigervnc-viewer
    usermod -a -G ubridge,libvirt,kvm,wireshark $USER
    else
	add-apt-repository ppa:gns3/ppa && apt update && apt install gns3-gui tigervnc-viewer
...
function TAP()
{
    apt install iptables-persistent dnsmasq
...
PPA "Necesario em sistemas Debian ou Deepin" \
GNS3 "Instala o GNS3" \
...
case $menuitem in
	...
	TAP) dialog --title "TAP setup" --clear \
        ...
        exemplo 192.168.0.0" 16 60 2> ${INPUT1}
        INPUTC=$(<"${INPUT1}")
        INPUT2=${INPUTC%??};
    TAP;;
	Sair) echo "Tchao"; break;;
esac
